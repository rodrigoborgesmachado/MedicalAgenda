...
namespace Agenda_de_Saude.Modelo
{
    public class MDN_Field
    {
        ...
        public MDN_Field(string field_name, bool notnull, DataType type, object default_value, bool primary, bool unique, int size, int precision)
        {
        ...
        public string InsertFieldDataBase()
        {
            ...
            switch (this.type)
            {
                ...
                case DataType.DATE:
                    command_sql += " INTEGER ";
                    command_sql += (notnull ? " DEFAULT " + Util.Conection.Date_to_Int(DateTime.Parse((this.ValueDefault == null ? 0.ToString() : this.ValueDefault.ToString()))) + " NOT NULL " : (unique ? "UNIQUE" : ""));
                    break;
